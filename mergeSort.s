...
; Spring 2018
; mergeSort.s
; Hunter Vaccaro (hpv8hf) 11/5/2019 	
...
mergeSort:
	...
	cmp rsi, rdx		; check if left index = right index
	je done			; if equal then done
	push rdx 		; saves right index to the stack
	add rdx, rsi		; add left to right
	shr rdx, 1		; divide it by 2
	call mergeSort		; merge sort left and middle
	pop rdx			; pops the right index back in
	push rsi		; saves left index to the stack
	add rsi, rdx		; add right to left
	shr rsi, 1 		; divides it by 2
	add rsi, 1		; adds one to avoid the (left+right)/2 thats already been done
	call mergeSort		; merge sort middle+1 and right
	pop rsi			; pops the left index back in
	mov rcx, rdx		; moves the right index to rcx
	push rdx		; saves the right index to the stack
	add rdx, rsi		; adds left to right index
	shr rdx, 1		; divide by 2
	push r10		; puts r10 into the stack
	push r11	        ; puts r11 into the stack	
	call merge		; merge (array, left , middle, right)
	pop r11			; brings r11 back
	pop r10			; brings r10 back
	pop rdx			; brings back the right index to the rdx
done:
	ret			; finishes
